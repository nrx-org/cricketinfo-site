...
// Remove after adding places file
// eslint-disable-next-line dot-notation
CONTENT_URLS["en"]["India"] = "/static/content/en/india.json";
...
export const getMainPageUrl = lang => `${getBaseUrl()}/${lang}`;
...
export const getAbsoluteArticleUrl = (articleId, lang) => {
  const absoluteBaseUrl = process.env.NOW_REGION
    ...
    : "http://localhost:3000";
  return `${absoluteBaseUrl}/${lang}/${articleId}`;
};

export const articleUrl = (articleId, lang) => {
  return `${getBaseUrl()}/${lang}/${articleId}`;
};
